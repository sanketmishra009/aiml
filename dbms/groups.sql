...
where
    market_date = '2019-05-01'

Select round(avg(BMI), 2) as avg_bmi
from `hospital`
where
    lower(Medical_Condition) = "obesity"
...
select extract(
        month
        from market_date
    ) as month, sum(quantity) as sales
from customer_purchases
group by
    1
order by 1;
...
select *, lag(mom, 1) over (
        order by quarter
    ) as shifter_mom
from (
        SELECT
            quarter, total_sales, LEAD(total_sales, 1) OVER (
                ORDER BY quarter
            ) AS next_quarter_sales, (
                LEAD(total_sales, 1) OVER (
                    ORDER BY quarter
                ) - total_sales
            ) / total_sales * 100 AS mom
        FROM (
                SELECT quarter, SUM(quantity) AS total_sales
                FROM (
                        SELECT NTILE(4) OVER (
                                ORDER BY EXTRACT(
                                        MONTH
...
select *, (next_higher_salary - salary) as sal_diff
from (
        select *, lead(salary, 1) over (
                order by salary
